...
export default function Home() {
  ...
  const serviceGradients = {
    ...
    sea: "from-sky-500 to-blue-600",
    rail: "from-sky-400 to-sky-600",
    road: "from-sky-500 to-blue-500",
    transit: "from-sky-400 to-blue-500",
    export: "from-sky-500 to-blue-600",
  };
  const serviceHrefs = {
    air: "/services/air",
    sea: "/services/sea",
  ...
  const services = serviceKeys.map((key) => ({
    ...
    gradient: serviceGradients[key],
  }));

  const additionalServiceKeys = ["insurance", "customs", "domestic"] as const;
  const additionalServiceHrefs = {
    insurance: "/services/insurance",
    customs: "/services/customs",
    domestic: "/services/domestic",
  };

  const additionalServices = additionalServiceKeys.map((key) => ({
    key,
    title: t(`additionalServices.${key}.title`),
    description: t(`additionalServices.${key}.description`),
  ...
  const resourceHrefs = {
    ...
    wagons: "/resources/wagons",
    dangerousGoods: "/resources/dangerous-goods",
    ...
    <>
        ...
        >
          ...
          >
            {[
              ...
              { value: "6", label: t("stats.modes") },
              { value: "24/7", label: t("stats.monitoring") },
              { value: "100%", label: t("stats.transparency") },
            ].map((stat, index) => (
              <motion.div
                key={stat.label}
      ...
      <section className="relative px-6 py-12 md:py-16">
        <div className="mx-auto max-w-7xl">
          <AnimatedSection className="mb-8 text-center md:mb-16">
            <motion.span className="inline-block rounded-full bg-sky-50 px-4 py-2 text-sm font-medium text-sky-600">
              {t("resourcesSection.badge")}
            </motion.span>
            <h2 className="mt-6 text-2xl font-bold text-slate-900 sm:text-3xl md:text-4xl">
              {t("resourcesSection.title")}{" "}
              <span className="bg-gradient-to-r from-sky-500 to-blue-600 bg-clip-text text-transparent">
                {t("resourcesSection.titleHighlight")}
              </span>
            </h2>
          </AnimatedSection>
